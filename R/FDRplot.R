...
#' @param outfile either 'FALSE' or the path/name of a new pdf file to be generated by the function. 
...
#' @export
FDRplot <-
function(plotdat,lowerbound,upperbound,ymax=1, annot="",xpos=.8, ypos=.8,outfile=FALSE){
	...
	##### myfdrplot.pdf
	if(outfile != FALSE) pdf(outfile, width = 7, height=7)
	...
	p1 = ggplot2::ggplot(plotdat, aes(x=threshold)) +
		...
		ggplot2::xlab(expression(paste('significance threshold (-',log[10],'(median(p-value)))'))) +
		ggplot2::ylab("FDR") + 
		ggplot2::labs(title="") +
